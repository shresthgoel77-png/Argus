...
def test_compute_and_persist_health_critical_finding(monkeypatch):
    ...
    prev = RepositoryHealthSnapshot(overall_score=100, category_scores={"security": 100, "ci_cd": 100, "dependencies": 100, "issues": 100, "pull_requests": 100, "code_quality": 100})
...
def test_compute_and_persist_health_resolved_finding(monkeypatch):
    ...
    prev = RepositoryHealthSnapshot(overall_score=60, category_scores={"security": 60, "ci_cd": 100, "dependencies": 100, "issues": 100, "pull_requests": 100, "code_quality": 100})
    monkeypatch.setattr("app.services.health_service.get_latest_snapshot", lambda db, repo: prev)
...
def test_compute_and_persist_health_no_changes(monkeypatch):
    ...
    prev = RepositoryHealthSnapshot(overall_score=93, category_scores={"security": 60, "ci_cd": 100, "dependencies": 100, "issues": 100, "pull_requests": 100, "code_quality": 100})
    monkeypatch.setattr("app.services.health_service.get_latest_snapshot", lambda db, repo: prev)
...
def test_health_drop_notification_thresholds(monkeypatch):
    import app.services.health_service
    from app.services import notification_dispatch_service
    
    dispatched = []
    def mock_dispatch(*args, **kwargs):
        dispatched.append(kwargs)
    monkeypatch.setattr(notification_dispatch_service, "dispatch_notification", mock_dispatch)
    
    monkeypatch.setattr("app.services.health_service.get_all_open_findings_for_repository", lambda db, repo: [])
    monkeypatch.setattr("app.services.health_service.compute_category_scores", lambda f: {})
    ...
    # 1. 14 point drop
    monkeypatch.setattr("app.services.health_service.compute_overall_score", lambda c: 86)
    ...
    monkeypatch.setattr("app.services.health_service.compute_overall_score", lambda c: 85)
    monkeypatch.setattr("app.services.health_service.get_latest_snapshot", lambda db, repo: RepositoryHealthSnapshot(overall_score=100))
    compute_and_persist_health(get_mock_db(), uuid.uuid4())
    ...
    dispatched.clear()
    monkeypatch.setattr("app.services.health_service.compute_overall_score", lambda c: 100)
    monkeypatch.setattr("app.services.health_service.get_latest_snapshot", lambda db, repo: RepositoryHealthSnapshot(overall_score=80))
    compute_and_persist_health(get_mock_db(), uuid.uuid4())
    assert len(dispatched) == 0
...
def test_health_drop_notification_exception_handling(monkeypatch):
    ...
    monkeypatch.setattr(notification_dispatch_service, "dispatch_notification", mock_dispatch_raise)
