...
def test_verify_end_to_end_health_drop(db_session, repo_for_findings, monkeypatch):
    ...
    snap1 = compute_and_persist_health(db_session, repo_for_findings.id)
    nots1 = db_session.query(Notification).all()
    print(f"\n1. First-ever snapshot computed.")
    print(f"   -> Notifications created: {len(nots1)} (Expected: 0)")
    
    # 2. Improvement
    snap_bad = RepositoryHealthSnapshot(repository_id=repo_for_findings.id, overall_score=50, category_scores={"security":50, "ci_cd": 50})
    db_session.add(snap_bad)
    db_session.commit()
    
    snap2 = compute_and_persist_health(db_session, repo_for_findings.id)
    ...
    f_med = Finding(repository_id=repo_for_findings.id, category="security", type="secret", title="t", description="d", severity="medium", priority="medium", source="s", status="open", evidence={})
    db_session.add(f_med)
    db_session.commit()
    
    snap3 = compute_and_persist_health(db_session, repo_for_findings.id)
    ...
    for cat in ["security", "ci_cd", "dependencies", "issues", "pull_requests", "code_quality"]:
        for i in range(2):
            findings.append(Finding(repository_id=repo_for_findings.id, category=cat, type=f"type{i}", title="t", description="d", severity="critical", priority="critical", source="s", status="open", evidence={}))
    db_session.add_all(findings)
    db_session.commit()
    
    db_session.add(RepositoryHealthSnapshot(repository_id=repo_for_findings.id, overall_score=100))
    db_session.commit()
    
    snap4 = compute_and_persist_health(db_session, repo_for_findings.id)
    ...
    snap5 = compute_and_persist_health(db_session, repo_for_findings.id)
