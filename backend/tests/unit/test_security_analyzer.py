    ...
    mock_context.client.list_dependabot_alerts.return_value = [
        {
            ...
            "dependency": {"package": {"name": "django"}},
        },
        {
            "number": 2,
            "state": "dismissed",
...
async def test_analyzer_github_auth_error(security_analyzer, mock_context):
    mock_context.client.list_dependabot_alerts.side_effect = GitHubAuthError(
        "403 Forbidden"
    )
...
@pytest.mark.asyncio
async def test_analyzer_missing_fields_graceful(
    security_analyzer, mock_context
):
    mock_context.client.list_dependabot_alerts.return_value = [
        {
            "number": 99,
    ...
    context.client.list_dependabot_alerts.return_value = [
        {
            "number": 10,
            "state": "open",
            "security_advisory": {"summary": "High Risk", "severity": "high"},
