...
async def test_analyzer_branch_protected(
    mock_get_branch_protection, code_quality_analyzer, mock_context
):
    ...
    mock_get_branch_protection.assert_called_once_with(
        999, "test/repo", "main"
    )
...
@pytest.mark.asyncio
@patch("app.monitoring.analyzers.code_quality_analyzer.get_branch_protection")
async def test_analyzer_branch_not_protected(
    mock_get_branch_protection, code_quality_analyzer, mock_context
):
...
@pytest.mark.asyncio
@patch("app.monitoring.analyzers.code_quality_analyzer.get_branch_protection")
async def test_analyzer_github_auth_error(
    mock_get_branch_protection, code_quality_analyzer, mock_context
):
...
@patch("app.monitoring.analyzers.code_quality_analyzer.get_branch_protection")
async def test_run_analyzer_verifies_sync_path_for_code_quality(
    mock_get_branch_protection, mock_client_class
):
    ...
    result = await run_analyzer(
        db_session, "code_quality", test_user_repository
    )
...
@pytest.mark.asyncio
@patch("app.monitoring.analyzers.code_quality_analyzer.get_branch_protection")
async def test_code_quality_analyzer_repository_scope(
    mock_get_branch_protection, code_quality_analyzer
):
